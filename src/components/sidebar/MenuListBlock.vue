...
import { isPlatform } from "@/util/platform.ts"
  ...
  ElMessageBox.confirm(
    ...
    .then(() => {
      if (isPlatform('window')) {
        window.open('http://192.168.32.60:3008/release/Chat-GPT-Client-v1.0.0-darwin-ia32-setup.exe', '_blank')
      } else if (isPlatform('mac')) {
        window.open('http://192.168.32.60:3008/release/Chat-GPT-Client-1.0.0-darwin.dmg', '_blank')
      }
    })
};
</script>
  ...
  <div class="menu-list-block p-2">
    <div
      ...
      <div
        ...
        <i class="iconfont icon-download text-xl transition ease-in-out group-hover:rotate-180 duration-700" />
      </div>
    </div>
