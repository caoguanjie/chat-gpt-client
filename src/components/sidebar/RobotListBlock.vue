  ...
  <div ref="robotListRefs" class="overflow-hidden overflow-y-auto">
    <div class="min-h-full max-h-0 p-1">
      <div
        ...
        v-for="(item, index) in robotList" :key="index" :class="index === activeRobotIndex ? 'robot-item-selected' : ''"
        @click="changeActiveRobot(index, item)">
        <div class="pr-1 flex-1 flex flex-row gap-1 items-center">
          <div class="flex-1 text-md leading-8 select-none overflow-hidden overflow-ellipsis whitespace-nowrap"
            :class="item.options.enabled ? 'max-w-[10rem]' : 'max-w-[16rem]'">
            {{ item.name }}
          </div>
          <el-tooltip :effect="tooltipEffect" :content="item.options.model?.toUpperCase()" placement="right"
            :hide-after="0" v-if="item.options.enabled" :enterable="false">
            <div
